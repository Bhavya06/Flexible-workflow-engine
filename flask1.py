...
app = Flask(__name__,static_url_path="/",static_folder='D:/Notes/CapstoneProject', template_folder='templates')

CORS(app, support_credentials=True)

class Engine(object):
    def __init__(self, name, start_node, current_node, total_time,last_node, priority_users):
        self.name = name
...
class Operator(object):
    def __init__(self, id, title, time, input, output, not_executed, fin_executed, jobs, is_executing):
        self.id = id
        self.title = title
        self.time = time
        self.input = input
...
def start():
    workflow_name = "TEST"
    globals()[workflow_name] = Engine(workflow_name, 'NULL', 'NULL', 0, 'NULL', ['User3'])
...
def add_operator():
    ...
    time = 0
    globals()[task_name] = Operator(task_name,'Task ' + str(operatorI + 1) + '[ Time: 0 ]', time,'NULL', 'NULL',[],[],0, False)      
...
def execute_node(current_node):
    if(current_node.is_executing == False):
        ...
        if(prev_link == 'NULL'):
            ...
            c=0
            while(current_node.not_executed[0] not in prev_node.fin_executed):
                c+=1
            while(current_node.jobs>0 and current_node.not_executed != [] and current_node.not_executed[0] in prev_node.fin_executed):
                not_exec = current_node.not_executed.pop(0)
                current_node.is_executing = True
                val = "Started execution of" +" "+current_node.title+" for "+not_exec+ "\n"
                yield "data: %s\n\n" % (val)
        ...
        def events():           
            ...
            while(current_node != 'NULL'):
                ...
                with concurrent.futures.ThreadPoolExecutor() as executer:
                    future = executer.submit(execute_node, current_node)
