...
jQuery(function($) {
    $.widget("flowchart.flowchart", {
        ...
        createLink: function(linkId, linkDataOriginal) {
            ...
            console.log('createLink', linkId, linkDataOriginal);
            add_link = {};
            add_link['linkId'] = linkId;
            add_link['linkDataOriginal'] = linkDataOriginal;
            $.ajax({
                type: "POST",
                url: 'http://localhost:5000/add_link',
        ...
        _deleteOperator: function(operatorId, replace) {
            ...
            deleted_data = {};
            deleted_data['operatorId'] = operatorId;
            $.ajax({
                type: "DELETE",
                url: 'http://localhost:5000/delete_operator',
                data: JSON.stringify(deleted_data),
                dataType: 'json'
            })
        ...
        _deleteLink: function(linkId, forced) {
            console.log('_deleteLink', linkId, forced);
            deleted_data = {};
            deleted_data['linkId'] = linkId;
            deleted_data['forced'] = forced;
            $.ajax({
                type: "DELETE",
                url: 'http://localhost:5000/delete_link',
                data: JSON.stringify(deleted_data),
                dataType: 'json'
            })
            if (this.selectedLinkId == linkId) {
        ...
        executeEnginestart: function(operatorId) {
            // this.data.operators[operatorId].internal.els.operator.css('background', '#fddd3a');
            var left = this.data.operators[operatorId].internal.els.operator.css('top');
            var top = this.data.operators[operatorId].internal.els.operator.css('left');
            return [left, top];
        },

        executeEnginecomplete: function(operatorId) {
            this.data.operators[operatorId].internal.els.operator.css('background', '#33dc2c');
            // var i = 0;
            // var op = 'created_operator_';
            // for (i = 0; i < 2; i++) {
            //     var temp = op + i;
            //     this.data.operators[temp].internal.els.operator.css('background', '#ee0a0a');
            //     // setTimeout($.proxy(function() {
            //     //     this.data.operators[temp].internal.els.operator.css('background', '#50dd0e');
            //     // }, this), 5000);
            //     this.sleep(5000);
            //     this.data.operators[temp].internal.els.operator.css('background', '#0a19ee');
            // }
            // this.data.operators['created_operator_0'].internal.els.operator.css('background', '#dd610e');
            //console.log(operatorId);
            //$(".flowchart-operator .flowchart-operator-title").css('background', '#d31088');
            //var valref = this;
            // setTimeout($.proxy(function() {
            //     this.data.operators['created_operator_0'].internal.els.operator.css('background', '#50dd0e');
            // }, this), 5000);

            // this.data.operators['created_operator_1'].internal.els.operator.css('background', '#dd610e');
            // //console.log(operatorId);
            // //$(".flowchart-operator .flowchart-operator-title").css('background', '#d31088');
            // setTimeout($.proxy(function() {
            //     this.data.operators['created_operator_1'].internal.els.operator.css('background', '#50dd0e');
            // }, this), 5000);
        },
